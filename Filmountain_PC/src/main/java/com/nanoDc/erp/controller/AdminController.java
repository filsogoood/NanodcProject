...
public class AdminController {
	...
	/*회원 관리 페이지*/ 
	 ...
	 public ModelAndView agreement_manager(HttpServletRequest request, Integer init_page) {
	     ...
	     // 사용자 정보 리스트 가져오기
	     List<AgreementVO> selectAgreementlist = this.adminService.selectAgreementlist();
	     List<UserInfoVO> userInfoList = adminService.selectUserInfoList();
	     List<UserInfoVO> activeUsers = userInfoList.stream()
	             .filter(user -> !"inactive".equals(user.getUser_status()))
	             .collect(Collectors.toList());
	     
	     List<UserInfoVO> usersWithNewContracts = activeUsers.stream()
	                .filter(user -> "신규계약".equals(user.getContract_status()))
	                .collect(Collectors.toList());
	     
	     Map<Integer, List<AgreementVO>> agreementMap = selectAgreementlist.stream()
	                .collect(Collectors.groupingBy(AgreementVO::getUser_id));
	     List<Object> mergedList = usersWithNewContracts.stream()
	                .map(user -> {
	                    Map<String, Object> mergedObject = new HashMap<>();
	                    mergedObject.put("user", user);
	                    mergedObject.put("agreements", agreementMap.getOrDefault(user.getUser_id(), Collections.emptyList()));
	                    return mergedObject;
	                })
	                .collect(Collectors.toList());
